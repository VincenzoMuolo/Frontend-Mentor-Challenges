...
function openCard(card) {
   ...
   document.getElementById(card.id + "-box").style.transition = "450ms opacity ease-in-out 200ms, 450ms transform ease-in-out";
   setTimeout(function rest() {
      document.getElementById(card.id + "-title").style.opacity = 1;
      document.getElementById(card.id + "-title").style.transition = "350ms opacity ease-in-out";
   }, 300);
   ...
   if (document.getElementById(card.id + "-challenge-container") != null) {
      ...
      document.getElementById(card.id + "-challenge-container").style.transition =
         "500ms width ease-in-out " + config.chContainerDelay[switchValue] + ", 500ms font-size ease-in-out " + config.chContainerDelay[switchValue];
   }

   /*
    ***********************TRANSITIONS***********************
    */
   newbie.style.transition =
      "500ms width ease-in-out, 500ms font-size ease-out," +
      "500ms height ease-in-out " +
      config.heightDelay[config.switchHeight[0 + position] * switchValue];

   junior.style.transition =
      "500ms width ease-in-out, 500ms font-size ease-out," +
      "500ms height ease-in-out " +
      config.heightDelay[config.switchHeight[0 + position] * switchValue];

   intermediate.style.transition =
      "500ms width ease-in-out, 500ms font-size ease-out," +
      "500ms height ease-in-out " +
      config.heightDelay[config.switchHeight[1 - position] * switchValue];

   advanced.style.transition =
      "500ms width ease-in-out, 500ms font-size ease-out," +
      "500ms height ease-in-out " +
      config.heightDelay[config.switchHeight[1 - position] * switchValue];
